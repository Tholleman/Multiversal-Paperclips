 ...
 * @returns {string}
 */
export function spellf(userInput) {
 ...
 * @returns {string}
 */
export function formatWithCommas(num, decimal = 0) {
	if (num === undefined) return '';
	if (decimal == 0) {
		return withCommas.format(num);
	} else {
...
export function addBreaksAtComma(string) {
	return string.replaceAll(',', ',<wbr>');
}

const below1e9=Intl.NumberFormat(undefined, {maximumFractionDigits: 0, signDisplay: 'exceptZero'});
const above1e9=Intl.NumberFormat(undefined, {minimumSignificantDigits: 3, maximumSignificantDigits: 3, notation: 'scientific'});

/**
 * @param {number} number 
 */
export function eInt(number) {
	// 999.999.999
	if (number < 1e9) {
		return below1e9.format(number);
	} else {
		return above1e9.format(number);
	}
}

const currencyBelow1e3=Intl.NumberFormat(undefined, {style: 'currency', currency: "USD", signDisplay: 'exceptZero'});
const currencyBelow1e9=Intl.NumberFormat(undefined, {style: 'currency', currency: "USD", maximumFractionDigits: 0, signDisplay: 'exceptZero'});
const currencyAbove1e9=Intl.NumberFormat(undefined, {style: 'currency', currency: "USD", minimumSignificantDigits: 3, maximumSignificantDigits: 3, notation: 'scientific'});

export function price(price) {
	if (price < 1e3) {
		return currencyBelow1e3.format(price);
	} else if (price < 1e9) {
		return currencyBelow1e9.format(price);
	} else {
		return currencyAbove1e9.format(price);
	}
}
